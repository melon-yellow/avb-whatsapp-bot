...
Avbot.add('cool_feedback', message => message.clean().match(/^\s*(obrigado|valeu)\s*$/), async (message) => {
  await message.quote('Estou as ordens! 😉🤝', 'cool_feedback');
});
// Producao Trefila
Avbot.add('prod_trf', message => message.clean().match(/^\s*(producao(\s+))?trefila\s*$/), async (message) => {
  await message.quote(Avbot.chat.gotIt, 'got_it');
  await message.send(Lam.getTref(), 'bot::prod_trf');
});
// Producao Laminador
Avbot.add('prod_lam', message => message.clean().match(/^\s*(producao(\s+))?laminador\s*$/), async (message) => {
  await message.quote(Avbot.chat.gotIt, 'got_it');
  await message.send(Lam.getProd(), 'bot::prod_lam');
});
// Producao do Mes Laminador
Avbot.add('prod_mes_lam', message => message.clean().match(/^\s*producao(\s+)(do(\s+))?mes((\s+)laminador)?\s*$/), async (message) => {
  await message.quote(Avbot.chat.gotIt, 'got_it');
